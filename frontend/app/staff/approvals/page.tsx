...
function StaffApprovalsContent() {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      if (json.success && json.data) {
        // Only load approved
        const meetingComplaints = json.data.filter((c: Complaint) => c.status === 'approved');
    ...
    try {
      const res = await api.get(`/complaints/all`);
        if (res.data.success && res.data.data) {
          const meetingComplaints = res.data.data.filter((c: Complaint) => c.status === 'approved');
          setComplaints(meetingComplaints);
        }
    } catch {}
  };
    ...
    <>
      ...
      <div className="space-y-6 animate-in fade-in duration-500 print-section">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-gray-200 pb-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800 m-0">รอเข้าที่ประชุม</h1>
            <p className="text-sm text-gray-400 mt-1 m-0 no-print">เลือกเรื่องร้องเรียนที่ได้รับอนุมัติเพื่อนำเรื่องเข้าวาระการประชุม</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="bg-purple-50/50 px-6 py-4 border-b border-purple-100 flex items-center justify-between no-print">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-purple-900 m-0">รายการที่รอการนำเข้าที่ประชุม</p>
                  <p className="text-xs text-purple-600 m-0 mt-0.5">กรุณาเลือกรายการด้านล่างเพื่อนำเรื่องเข้าสู่ที่ประชุม</p>
                </div>
              </div>
            </div>
            ...
            <div className="fixed inset-0 z-50 flex justify-end">
              ...
              <div className="relative w-full max-w-sm sm:max-w-md h-full bg-white/95 backdrop-blur-xl shadow-[0_0_40px_rgba(0,0,0,0.1)] flex flex-col animate-in slide-in-from-right duration-300 border-l border-white/50">
                <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-bold text-gray-800 m-0">เตรียมวาระการประชุม</h3>
