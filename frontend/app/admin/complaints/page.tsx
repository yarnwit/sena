    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:flex-wrap items-stretch md:items-center justify-between gap-3">
        ...
        <Link
          ...
          className="hidden w-full md:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-[#B31B1B] hover:bg-red-800 text-white rounded-xl text-sm font-medium no-underline transition-colors shrink-0"
        >
        ...
        {filterOptions.map((option) => (
          <button
            ...
            className={`
              ...
              ${filter === option.key
                ? "bg-[#B31B1B] border-[#B31B1B] text-white"
                : "bg-white border-gray-200 text-gray-500 hover:border-[#B31B1B] hover:text-[#B31B1B]"
              }
            `}
                ...
                {filtered.map((c) => {
                  ...
                  return (
                    <tr
                      ...
                      onClick={() => window.location.href = `/admin/complaints/${c.complaint_id}`}
                    >
                      <td className="px-6 py-4 font-semibold text-[#B31B1B] text-sm">
                      ...
                      <td className="px-6 py-4">
                        <Link
                          ...
                          className="text-[#B31B1B] hover:text-red-800 text-xs font-medium no-underline transition-colors"
            ...
            {filtered.map((c) => {
              ...
              return (
                <Link
                  key={c.complaint_id}
                  href={`/admin/complaints/${c.complaint_id}`}
                  className="block bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4 no-underline hover:border-red-200 transition-colors"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-[#B31B1B] m-0 mb-1">
                        {c.ticket_no || `#${c.complaint_id}`}
                      </p>
...
export default function AdminComplaintsPage() {
  return (
    <Suspense fallback={
      <div className="flex items-center justify-center py-32">
        <div className="w-10 h-10 border-3 border-gray-200 border-t-[#B31B1B] rounded-full animate-spin" />
      </div>
    }>
