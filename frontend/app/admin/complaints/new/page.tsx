...
export default function AdminNewComplaintPage() {
  ...
  const executeSubmit = async () => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push("/admin/complaints"), 2000);
  ...
  if (pageLoading) return <div className="flex items-center justify-center py-32"><div className="w-10 h-10 border-3 border-gray-200 border-t-[#B31B1B] rounded-full animate-spin" /></div>;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {/* Header */}
        <div className="flex items-start gap-4 px-6 sm:px-8 py-6 bg-gradient-to-r from-red-50 to-red-100 border-b border-red-200">
          <div className="w-12 h-12 rounded-2xl bg-red-100 flex items-center justify-center shrink-0">
            <svg className="w-6 h-6 text-[#B31B1B]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
              <polyline points="14 2 14 8 20 8" />
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-8">
          ...
          <div>
            ...
            <div className="flex gap-2 mb-5">
              <button type="button" onClick={() => { setMode("select"); setError(""); }}
                className={`px-5 py-2.5 rounded-xl text-sm font-medium border transition-all cursor-pointer ${mode === "select" ? "bg-[#B31B1B] text-white border-[#B31B1B]" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"}`}>
                เลือกจากระบบ
              </button>
              <button type="button" onClick={() => { setMode("manual"); setError(""); }}
                className={`px-5 py-2.5 rounded-xl text-sm font-medium border transition-all cursor-pointer ${mode === "manual" ? "bg-[#B31B1B] text-white border-[#B31B1B]" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"}`}>
                กรอกข้อมูลเอง
              </button>
            </div>
                ...
                <div className="max-h-48 overflow-y-auto rounded-xl border border-gray-200 bg-white divide-y divide-gray-50">
                  {filteredResidents.length === 0 ? (
                    ...
                    <button key={r.resident_id} type="button" onClick={() => setSelectedResidentId(r.resident_id)} disabled={loading || success}
                      className={`w-full flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 px-4 py-3 text-left transition-colors cursor-pointer border-none ${selectedResidentId === r.resident_id ? "bg-red-50 ring-1 ring-red-300" : "bg-white hover:bg-gray-50"}`}>
                      <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-50 text-red-700 font-semibold text-xs shrink-0">🏠 {r.house_no || "-"}</span>
                      ...
                      <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto sm:ml-auto">
                        ...
                        {selectedResidentId === r.resident_id && <span className="text-[#B31B1B] font-bold sm:ml-2">✓</span>}
                      </div>
                    </button>
                  ))}
                </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <Link href="/admin/complaints" className="px-8 py-3 rounded-xl border border-gray-200 bg-white text-sm font-medium text-gray-600 hover:bg-gray-50 no-underline transition-colors">ยกเลิก</Link>
            <button type="submit" className="px-8 py-3 rounded-xl bg-[#B31B1B] hover:bg-red-800 text-white text-sm font-medium border-none cursor-pointer transition-colors disabled:opacity-50" disabled={loading || success}>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
            ...
            <p className="text-sm text-gray-500 mb-8">สร้างคำร้องให้ <strong className="text-[#B31B1B]">{confirmLabel}</strong></p>
