...
interface AuthContextType extends AuthState {
  login: (data: LoginPayload, expectedRole?: string) => Promise<void>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
   ...
   */
  const login = useCallback(async (data: LoginPayload, expectedRole?: string) => {
    const response = await authApi.login(data);
    const { user: userData, accessToken, refreshToken } = response.data;
    
    // Validation: Check if actual role matches expected role from UI
    const actualRole = userData.role || 'resident';
    if (expectedRole && actualRole !== expectedRole) {
      throw new Error('บัญชีของคุณไม่สามารถเข้าสู่ระบบในสิทธิ์ที่เลือกได้');
    }
